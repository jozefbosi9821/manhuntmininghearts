...
public class AddHunterCommand implements CommandExecutor {
    ...
    public AddHunterCommand(ManhuntMiningHearts plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length < 1) {
            player.sendMessage("Usage: /addhunter <player>");
            return true;
        }
        ...
        teamManager.addHunter(targetPlayer);
        player.sendMessage(targetPlayer.getName() + " has been added to the Hunter team.");
