...
public class RemoveHunterCommand implements CommandExecutor {
    ...
    public RemoveHunterCommand(ManhuntMiningHearts plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length < 1) {
            player.sendMessage("Usage: /removehunter <player>");
            return true;
        }
        ...
        teamManager.removeHunter(targetPlayer);
        player.sendMessage(targetPlayer.getName() + " has been removed from the Hunter team.");
